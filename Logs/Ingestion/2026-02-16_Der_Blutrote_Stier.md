# 📥 Ingestion Report: Der Blutrote Stier (Tai Sah'Halour - Glurias)

## Metadaten
- **Quelle**: Der Blutrote Stier.md (`../../Quellen/Bibliothek%20Astrael/Der%20Blutrote%20Stier.md`)
- **Datum der Verarbeitung**: 2026-02-16
- **Quellentyp**: #überlieferung
- **Lore-Score (LQS)**: 7/10
- **Verantwortlicher Agent**: Antigravity

## 📊 Lore Quality Score (LQS)
| Kriterium | Score (0-3) | Notiz |
|---|---|---|
| **Abdeckung** | 3 | Vollständige Wiedergabe der Legende |
| **Tiefe** | 2 | Mythologische Tiefe über die Gefahren von Hochmut |
| **Konsistenz** | 3 | Passt in den Kanon der nordmännischen Sagen |
| **Gesamt (LQS)** | **7/10** | (Wichtige kulturelle Sage der Su'un) |

## 📋 Extraktions-Ergebnisse

### 👤 Personen
| Name | Status | Ziel-Datei | Confidence | Notiz |
|---|---|---|---|---|
| [[Tai_Sah'Halour]] | NEU | [[Tai_Sah'Halour]] | 10 | Autor, eventuell ein Gelehrter oder Barde |
| [[Glurias]] | NEU | [[Glurias]] | 10 | Co-Autor oder Titel |

### 🗺️ Orte / 🐉 Bestiarium / 🔮 Konzepte
| Name | Kategorie | Status | Wiki-Link | Notiz |
|---|---|---|---|---|
| [[Skapen]] | Ort | ERWÄHNT | [[Skapen]] | Gebirge im Norden |
| [[Su'un]] | Volk | UPDATE | [[Su-un]] | Nordmänner-Stämme |
| [[Roter_Stier]] | Wesen | NEU | [[Der_Blutrote_Stier]] | Manifestation eines blutigen Wunsches |
| [[Der_bunte_Fremde]] | Wesen | NEU | [[Religion_Übersicht]] | Eventuell eine Entität wie Galtor |

## 🧠 Lore-Audit & Narrative Highlights
- **Wichtigste Erkenntnisse**: Die Sage warnt vor dem Preis von Machtwünschen und dem Blutvergießen im Namen der Krone. Der "bunte Fremde" agiert als klassischer Trickster.
- **Inkonsistenzen**: Keine.
- **Highlight**: Die groteske Beschreibung des Stiers als Ansammlung hunderter blutiger Leiber.

---
*Report-ID: i0j1k2l3-m4n5-6o7p-8q9r-0s1t2u3v4w5x*
*Status: COMPLETED*
