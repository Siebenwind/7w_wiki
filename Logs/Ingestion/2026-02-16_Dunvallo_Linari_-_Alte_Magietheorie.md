# 📥 Ingestion Report: Dunvallo Linari - Alte Magietheorie (Dunvallo Linari)

## Metadaten
- **Quelle**: Dunvallo Linari - Alte Magietheorie.md (`../../Quellen/Bibliothek%20Toran%20Dur/Dunvallo%20Linari%20-%20Alte%20Magietheorie.md`)
- **Datum der Verarbeitung**: 2026-02-16
- **Quellentyp**: #überlieferung (Historisches Standardwerk der Magie)
- **Lore-Score (LQS)**: 10/10
- **Verantwortlicher Agent**: Antigravity

## 📊 Lore Quality Score (LQS)
| Kriterium | Score (0-3) | Notiz |
|---|---|---|
| **Abdeckung** | 3 | Umfassendes Werk über Gitter-Theorie und Artefaktbau |
| **Tiefe** | 3 | Detaillierte mathematische und praktische Anweisungen |
| **Konsistenz** | 3 | Eckpfeiler der Siebenwind-Magietheorie |
| **Gesamt (LQS)** | **10/10** | (Absolutes Basiswerk) |

## 📋 Extraktions-Ergebnisse

### 👤 Personen
| Name | Status | Ziel-Datei | Confidence | Notiz |
|---|---|---|---|---|
| [[Dunvallo_Linari]] | UPDATE | [[Dunvallo_Linari]] | 10 | Erzmagus, Verfasser der Gitter-Theorie |
| [[Santanos_Alexandrius_von_Eichstatt]] | UPDATE | [[Santanos_Alexandrius]] | 10 | Magister Magnus, Herausgeber/Kommentator der alten Schrift |

### 🔮 Konzepte
| Name | Kategorie | Status | Wiki-Link | Notiz |
|---|---|---|---|---|
| [[Thaumaturgisches_Gitter]] | Konzept | UPDATE | [[Magie]] | Modell aus Fäden und Knoten zur Beschreibung von Mana |
| [[Knotentheorie]] | Konzept | NEU | [[Magie]] | Die Idee, dass Knoten die eigentliche Energiequelle sind |
| [[Artefakterstellung]] | Fertigkeit | UPDATE | [[Artefakte]] | Dreiteiliger Aufbau: Klinge, Handstück (Focus), Knauf (Elementstein) |
| [[Tunneleffekt]] | Phänomen | NEU | [[Magie]] | Ansaugen von Mana aus anderen Sphären mittels Untoten-Fokus |

## 🧠 Lore-Audit & Narrative Highlights
- **Wichtigste Erkenntnisse**: Linari etabliert das Gittermodell als Standard für die Artefakterstellung. Ein Schwert muss demnach aus einer Klinge (Runen), einem Handstück (Focus aus Untotenknochen) und einem Knauf (Edelstein zur Elementbindung) bestehen. Der "Tunneleffekt" erklärt, wie Artefakte Energie aus anderen Sphären beziehen.
- **Inkonsistenzen**: Er merkt an, dass die "Elementarknoten" auch andersherum betrachtet werden können (Knoten vor Fäden).
- **Kuriosum**: Die Verwendung von "Untotenknochen" als besten Fokus für den Tunneleffekt.

---
*Report-ID: l1n4r1-m4g-2026*
*Status: COMPLETED*
