# 📥 Ingestion Report: Dunvallo Linari - Matrixtheorie (Dunvallo Linari / Sheddja)

## Metadaten
- **Quelle**: Dunvallo Linari - Magietheoretische Grundlagen zur Zauberwirkung Matrixtheorie.md (`../../Quellen/Bibliothek%20Toran%20Dur/Dunvallo%20Linari%20-%20Magietheoretische%20Grundlagen%20zur%20Zauberwirkung%20Matrixtheorie.md`)
- **Datum der Verarbeitung**: 2026-02-16
- **Quellentyp**: #überlieferung (Technische Magietheorie / Verschiedene Quellen)
- **Lore-Score (LQS)**: 10/10
- **Verantwortlicher Agent**: Antigravity

## 📊 Lore Quality Score (LQS)
| Kriterium | Score (0-3) | Notiz |
|---|---|---|
| **Abdeckung** | 3 | Beinhaltet Matrix-Definitionen, Blutmagie und Elementar-Erweiterungen |
| **Tiefe** | 3 | Unterscheidung zwischen Matrix naturalis, artificialis und arcana |
| **Konsistenz** | 3 | Konsistent mit den dunklen und hellen Seiten der Magieforschung |
| **Gesamt (LQS)** | **10/10** | (Essenzielle theoretische Quelle) |

## 📋 Extraktions-Ergebnisse

### 👤 Personen
| Name | Status | Ziel-Datei | Confidence | Notiz |
|---|---|---|---|---|
| [[Sheddja]] | NEU | [[Sheddja]] | 10 | Meisterin, Autorin des Matrix-Schreibens |
| [[Okallus_Barim]] | NEU | [[Okallus_Barim]] | 9 | Magier der dunklen Künste, früherer Besitzer des Folianten |

### 🔮 Konzepte
| Name | Kategorie | Status | Wiki-Link | Notiz |
|---|---|---|---|---|
| [[Matrix_naturalis]] | Konzept | NEU | [[Magie]] | Das natürliche, unsymmetrische Mana-Netz |
| [[Matrix_artificialis]] | Konzept | NEU | [[Magie]] | Vom Magier konstruierte Matrix für Zauber |
| [[Matrix_arcana]] | Konzept | NEU | [[Magie]] | Matrix magischer Körper (Metalle, Steine, Geist) |
| [[Blutmagie]] | Magiezweig | UPDATE | [[Magie]] | Nutzung der Lebenskraft (Sikaryan) aus dem Blut |
| [[Siebtes_Element]] | Theorie | NEU | [[Kosmologie]] | Die Magie selbst als umstrittenes siebtes Element |
| [[Limbus]] | Ort | ERWÄHNT | [[Limbus]] | Raum zwischen den Welten / Zwiebelschalenmodell |

## 🧠 Lore-Audit & Narrative Highlights
- **Wichtigste Erkenntnisse**: Das Werk definiert die Matrix als notwendiges Medium für jeden Zauber. Es wird dargelegt, wie Lebensenergie (Sikaryan) aus frischem Blut (insb. Herz, Gehirn) genutzt werden kann, um Zauber zu verstärken, wobei vor den ethisch-metaphysischen Folgen gewarnt wird ("Götter lassen Frevler nicht ungestraft"). Die Sphärentheorie nach Nembiuz (Parallelwelten) wird ebenfalls diskutiert.
- **Highlight**: Die Theorie des 7. Elements (Magie) und die Oktalogie-Ansätze.
- **Inkonsistenzen**: Linari selbst merkt an, dass der Text veraltet sein mag, da er Modelle als Wirklichkeit darstellt.

---
*Report-ID: l1n4r1-m4t-2026*
*Status: COMPLETED*
