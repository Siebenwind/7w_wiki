...
def rules() -> list[FamilyRule]:
    return [
        ...
        FamilyRule(
            id="archive_inventory_reports",
            root=REPO_ROOT / "Logs" / "Archive",
            pattern="INVENTORY_*.json",
            classification="versioned_cold",
            reason="Archive inventory floods are evidence tails and belong in colder buckets after the latest window.",
            action="move_to_cold",
            cold_root=ARCHIVE_COLD_ROOT,
            keep_latest=RETENTION_LIMIT,
            skip_names=("INVENTORY_latest.json",),
        ),
        FamilyRule(
            id="archive_queue_reports",
            root=REPO_ROOT / "Logs" / "Archive",
            pattern="QUEUE_*.json",
            classification="versioned_cold",
            reason="Queue evidence is useful for forensics, but the hot archive only needs the newest rolling tail.",
            action="move_to_cold",
            cold_root=ARCHIVE_COLD_ROOT,
            keep_latest=RETENTION_LIMIT,
            skip_names=("QUEUE_latest.json",),
        ),
        FamilyRule(
            id="audit_reports",
            root=REPO_ROOT / "Logs" / "Archive",
        ...
        FamilyRule(
            id="archive_permission_temp",
            root=REPO_ROOT / "Logs" / "Archive",
            pattern=".perm_test_*",
            classification="local_runtime",
            reason="Permission probe leftovers in the archive are disposable temp artifacts, not historical evidence.",
            action="prune_runtime_tree",
        ),
        FamilyRule(
            id="archive_permission_log",
            root=REPO_ROOT / "Logs" / "Archive",
            pattern="test_permissions.txt",
            classification="local_runtime",
            reason="Permission test outputs are temporary diagnostics and should not remain in the hot archive surface.",
            action="prune_runtime_tree",
        ),
    ]
