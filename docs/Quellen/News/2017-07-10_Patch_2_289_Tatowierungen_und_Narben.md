---
source: https://www.siebenwind.de/patch-2-289-taetowierungen-und-narben/
title: Patch 2.289 + Tätowierungen und Narben
date: 2017-07-10
type: Patch
epistemic: "#news"
status: archiviert
post_id: 7647
kategorien:
  - Allgemein
  - Patch
  - Siebenwindnews
---

# Patch 2.289 + Tätowierungen und Narben

_Beobachterprotokoll des Netz-Waechters. Quelle: offizielle Siebenwind-News._

## Meldungskern

Aloha werte Spielerschaft Wie gewohnt gibt es einige Static- und Mapänderungen: Spielertickets Eventbauten Baustelle Ritterburg im Finalmodus Baustelle Brandenstein zu 80% fertig /  50% Umsetzung folgt im Folgepatch in wenigen Tagen kleinere Gumpänderungen   Unter Gumpänderungen fallen vor Allem neu eingepflegte Tattoos, die absofort im Spiel greifbar sind. Diese Tätowierungen sind färbbar. Für die Gesichtstattoos […]

## Volltext (bereinigt)

Aloha werte Spielerschaft

Wie gewohnt gibt es einige Static- und Mapänderungen:

Spielertickets
Eventbauten
Baustelle Ritterburg im Finalmodus
Baustelle Brandenstein zu 80% fertig /  50% Umsetzung folgt im Folgepatch in wenigen Tagen
kleinere Gumpänderungen

 

Unter Gumpänderungen fallen vor Allem neu eingepflegte Tattoos, die absofort im Spiel greifbar sind. Diese Tätowierungen sind färbbar. Für die Gesichtstattoos wird es künftig noch eine „Ausziehmöglichkeit“ geben wie bei Schmuck.

 

Tätowierungen und Narben

Tätowierung: Unter Tätowierung fallen alle Körperbemalungen, die permanent auf dem Körper aufgemalt werden.

Narben: Unter Narben fallen alle Schmuck- oder Brandnarben, die auf dem Körper dargestellt werden.

Tätowierungen und Narben müssen im Paperdoll beschrieben werden und dürfen nicht an andere Spieler weitergegeben werden
Tätowierungen und Narben kann man sich im Laufe des Spieles selbst aneignen
Tätowierungen und Narben müssen gemeldet werden, sodass sie als Sonderitem für den jeweiligen Charakter eingetragen werden
Tätowierungen und Narben können nach drei Monaten wieder entfernt werden
Rassentätowierungen dürfen nur von der jeweiligen Rasse gemalt/gestochen werden und können bei gutem RP-Grund auch an eine andere Rasse gegeben werden. Dazu muss ein Ticket an den Support mit dem Spieler, der dass Tattoo sticht, dem Empfänger und dem RP Grund geschrieben werden.

Eine Tätowierung kann per Call beim Support angefordert werden. Dazu muss der Charakter 50 Bandagen, 2 Dolche und 2 Feder und Tinte als Tauschitem im Rucksack haben. Für Narben selbst wird kein Tauschitem benötigt.

 

MfG,

Euer Siebenwind Staff

Siehe auch: [[Die_Chronik]]
