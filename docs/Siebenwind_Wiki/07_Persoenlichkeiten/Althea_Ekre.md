---
uuid: e5f6a7b8-c9d0-4e1f-2a3b-4c5d6e7f8a9b
title: Althea Ekre
category: Persönlichkeit
quelle: ../../Quellen/Zeitung 7w Bote/Siebenwind_Bote_180.md
letzter_check: 2026-02-13T22:30:00
report_id: 754c8f86-07f6-4769-ad51-79434539af3e
---

# Althea Ekre

!!! info "Metadaten"
    - **Titel:** Magistra
    - **Epistemischer Status:** #bote
    - **Zugehörigkeit:** Wissenschaft / Akademie (galadonisch)


## Beschreibung
**Althea Ekre** ist eine Magistra, die im Jahr 21 n.H. in Berichten über akademische oder administrative Angelegenheiten erwähnt wird (Bote 180).

## Wirken
- **Status:** Anerkannte Gelehrte mit dem Titel Magistra.

## Verlinkte Themen
- [[Personenregister]]

## Referenzen
- Primärquelle: [[Siebenwind_Bote_180]](../../Quellen/Zeitung%207w%20Bote/[[Siebenwind]]%20Bote%20180.md)
