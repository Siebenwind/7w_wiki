---
layout: wiki_page
title: Interessante Artikel
category: Archiv
---

# Interessante Artikel

Diese Seite ist die oeffentliche Kurations- und Releaseflaeche des Archivs.
Sie verbindet zwei Dinge:
- kuratierte Einstiegsstrecken fuer Leserinnen und Leser
- freigegebene Forschungs- und Archivresultate

Die Auswahl ist **teilautomatisiert**: Vorselektion per Qualitaetsdaten, finale Freigabe durch Redaktion.

## Neu im Archiv

### April 2026: Freigegebene Berichte und Archivresultate

| Neu veroeffentlicht | Typ | Stand | Warum jetzt lesenswert |
|---|---|---|---|
| [[Historian_Report_2026_003]] | Historian Report | 2026-04-08 | Gebuendelter Rueckblick auf Tiefenbach, Finsterwangen und das [[Ödland]] in einem publizierten Bericht. |
| [[RESEARCH-2026-009_Report]] | Research Report | 2026-04-08 | Schliesst die Strukturfrage zu Gilden und Handwerk in einer freigegebenen Ergebnisdarstellung. |
| [[RESEARCH-2026-008_Report]] | Research Report | 2026-04-08 | Verdichtet die Klaerung der `Dur`-Identitaet in einen publizierten Bericht. |
| [[Research_Board]] | Ausschreibungsboard | 2026-04-08 | Zeigt den aktuellen Stand offener, laufender und bereits publizierter Lore-Forschung. |
| [[Wiki_Statistiken]] | Statusseite | 2026-04-08 | Macht sichtbar, wie stark das Archiv gewachsen ist und welche Aktivitaet zuletzt stattfand. |

**Hinweis:** Laufende Historian-Faelle oder Seiten im Status `fachlich offen` erscheinen nicht als `Neu im Archiv`.
Sie bleiben auf [[Research_Board]] sichtbar, bis sie fachlich geklaert oder menschlich entschieden wurden.

**Publikationsregel ab April 2026:** Forschungsfortschritt bleibt nicht mehr nur im Dispatch oder in `Logs/`.
Jeder relevante Stand gehoert auf eine oeffentlich verlinkbare Archivseite:
- Ausschreibung und Status auf [[Research_Board]]
- laufender Erkenntnisstand auf der jeweiligen `[[RESEARCH-2026-...]]`-Seite
- Abschluss auf einer Report-, Dossier- oder Ergebnis-Seite
- kuratorische Sichtbarkeit ueber diese Seite und die Startseite

## Aktuelle Lesestrecke

### April 2026: Unter dem Weissen Hochturm

Diese Rotation fuehrt nicht ueber Register, sondern ueber Spannungslinien im Herzen des Reiches:
Stadt, Glaube, Ordnung, Geheimbund und die kleine Menschenperspektive im Schatten grosser Institutionen.

| Artikel | Kanon-Begruendung | Quellenbasis | Leserwert |
|---|---|---|---|
| [[Draconis]] | Draconis ist politisches, religioeses und kulturelles Machtzentrum Galadons und verbindet mehrere Kernachsen des Settings. | #canon + #bote; Stadtprofil mit Verweis auf [[Siebenwind_Bote_123]] | Ideal als Raumanker fuer neue Leser: von hier aus erschliessen sich Reich, Kirche und spaetere Konflikte. |
| [[Das_Pantheon|Das Pantheon]] | Die Gottesordnung ist Grundvokabular fuer weite Teile des Kanons und strukturiert Moral, Macht und Alltagsdeutung. | #canon; Verweisnetz auf Viere, Gohor und Gegenspieler | Schneller, klarer Einstieg in religioese Begriffe ohne sofort in Spezialartikel abzutauchen. |
| [[Codex_Iuris_Canonici|Codex Iuris Canonici]] | Der Codex zeigt, wie Glauben in Herrschaft, Recht und Kirchenhierarchie uebersetzt wird. | #ueberlieferung; Primaerquelle `Codex Iuris Canonici` | Leser erhalten nicht nur Mythos, sondern die konkrete Ordnung hinter Thron, Altar und Disziplin. |
| [[Ring_des_Argionemes|Ring des Argionemes]] | Der Artikel oeffnet die Perspektive auf inoffizielle Macht und innerkirchliche Strategie jenseits offizieller Dogmatik. | #perspektive; `Das_Ende_der_Zeit_der_Koenige` | Bietet Intrige, verdeckte Agenda und damit eine erzahlerisch starke Vertiefung nach den Grundlagen. |
| [[Arman]] | Arman verdichtet Vorurteil, Volksglauben und Ordensnaehe in eine einzelne, lesbare Figur. | #bote; [[Siebenwind_Bote_123]] | Menschlicher Einstieg in dieselbe Weltlage: weniger System, mehr unmittelbare Erfahrung. |

## Vorgeschlagene Archiv-Themenmotive

- **Morgenglocken ueber Draconis:** Heller Flussdunst, ferne Tuerme, nasses Steinpflaster, kaum Menschen, nur Andeutung von Groesse und Ordnung.
- **Schrift und Weihrauch:** Ein schwerer Eichentisch mit Pergament, Siegelwachs, angelaufenen Silberkanten und diffusem Tempellicht statt Techniksymbolik.
- **Die gruene Traene Vitamas:** Innenhof oder Herberge mit Brunnen, Reisetasche, schmalem Votivband und einem einzigen farbigen Akzent.
- **Verborgene Konklave:** Arkaden, Schatten, matte Wandmalerei und eine halb gehoerte Versammlung hinter Stoffbahnen, ohne Personen frontal auszustellen.

## Kurationsverfahren (teilautomatisiert)

1. **Vorselektion (maschinell):**
   - Kandidaten aus hochwertigen Kernseiten, stabilen Registerartikeln und neu publizierten Forschungsseiten sammeln.
   - Qualitaetsstand pro Kandidat mit `./7w_wiki.py score <datei>` pruefen.
   - Stil- und Strukturcheck mit `./7w_wiki.py check <datei>`.
2. **Kuratorische Pruefung (menschlich):**
   - Kanonische Relevanz
   - Leserwert (Einstiegsnutzen, Verstaendlichkeit)
   - Publikationswert (neue Erkenntnis, klare Verdichtung, sichtbarer Fortschritt)
   - Quellenbezug und Nachvollziehbarkeit
3. **Freigabe und Rotation:**
   - Monatlich bis zu drei neue Empfehlungen und bis zu fuenf sichtbare Neuveroeffentlichungen.
   - Aenderungen im Dispatch ankündigen und im Changelog dokumentieren.

## Qualitätskriterien

- Keine spekulativen Aussagen ohne Quellenbasis.
- Gute Verlinkung in Register und Chronik.
- Lesbarer Einstieg ohne Vorwissen.
- Forschungsseiten muessen den letzten belastbaren Stand sichtbar machen, auch wenn die Arbeit noch nicht abgeschlossen ist.
- Berichte gehoeren in das publizierte Archiv, nicht nur in `Logs/Research/`.

## Nächster Review-Termin

- Erste Kalenderwoche im Mai 2026.
