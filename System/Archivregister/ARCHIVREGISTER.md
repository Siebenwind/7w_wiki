---
layout: wiki_page
title: Archivregister
category: System
uuid: 134225f9-9322-4a80-8b1f-dc21917a0ea8
letzter_check: 2026-02-16T22:17:25Z
---

# Archivregister

**Epistemischer Status:** #meta

## Überblick

- Datensaetze gesamt: 3885
- Mit Content-UUID: 439 (11.30%)
- Wiki indexierbar: 1108 | Quellen indexierbar: 795

## RAG-Fortschritt

| Corpus | Indexierte Dateien | Indexierbare Dateien | Coverage | Chunks | Stale Index-Eintraege |
|---|---:|---:|---:|---:|---:|
| wiki | 158 | 1108 | 14.26% | 31700 | 0 |
| quellen | 288 | 300 | 96.00% | 60151 | 1 |

## Ingestion-Fortschritt (Inventur)

- INVENTUR_QUELLEN: 3/41 verarbeitet; Pending: 38

## Dispatch & Board Status

| Domain | Total | Status Breakdown | Missing UUID | Invalid Status Files |
|---|---:|---|---:|---:|
| Dispatch | 4 | OPEN=4 | 0 | 0 |
| research | 7 | DONE=2, TENDERED=3 | 7 | 2 |
| conflicts | 7 | AUTO_RESOLVED=1, NEEDS_REVIEW=4, RESOLVED=1 | 7 | 1 |
| inquisition | 3 | COMPLETED=1, OPEN=1, UNSPECIFIED=1 | 2 | 0 |

## Corpus-Verteilung

| Corpus | Dateien |
|---|---:|
| wiki | 1108 |
| quellen | 795 |
| system | 49 |
| docs | 1933 |

## Pflichtfelder je Datensatz

- `record_uuid` (deterministisch aus Pfad)
- `filename`
- `relative_path`
- `content_uuid` (falls Frontmatter vorhanden)
- `indexable`, `rag_source`, `modified_at`, `content_hash`

## Vollregister

- Siehe `System/Archivregister/ARCHIVREGISTER.json`

## UUID-Lücken (Markdown, Top 25)

- `Siebenwind_Wiki/00_Fundament/Adelskalender.md`
- `Siebenwind_Wiki/00_Fundament/Archiv_Register.md`
- `Siebenwind_Wiki/00_Fundament/Bestiarium_Register.md`
- `Siebenwind_Wiki/00_Fundament/Bibliotheks_Register.md`
- `Siebenwind_Wiki/00_Fundament/Das_Fundament.md`
- `Siebenwind_Wiki/00_Fundament/Die_Monde.md`
- `Siebenwind_Wiki/00_Fundament/Die_Sprache_Run.md`
- `Siebenwind_Wiki/00_Fundament/Dwarschim.md`
- `Siebenwind_Wiki/00_Fundament/Ecclesia.md`
- `Siebenwind_Wiki/00_Fundament/Galadon.md`
- `Siebenwind_Wiki/00_Fundament/Glossar.md`
- `Siebenwind_Wiki/00_Fundament/Handelskontore_der_Dwarschim_(H&H).md`
- `Siebenwind_Wiki/00_Fundament/Handwerk_Übersicht.md`
- `Siebenwind_Wiki/00_Fundament/Hochelfen.md`
- `Siebenwind_Wiki/00_Fundament/Inquisition.md`
- `Siebenwind_Wiki/00_Fundament/Königliche_Akademie_der_Kriegskünste.md`
- `Siebenwind_Wiki/00_Fundament/Lehensbanner.md`
- `Siebenwind_Wiki/00_Fundament/Linguistik_Übersicht.md`
- `Siebenwind_Wiki/00_Fundament/Link_Test.md`
- `Siebenwind_Wiki/00_Fundament/Löwenorden.md`
- `Siebenwind_Wiki/00_Fundament/Magie_Grundlagen.md`
- `Siebenwind_Wiki/00_Fundament/Magietheorie_Toran_Dur.md`
- `Siebenwind_Wiki/00_Fundament/Orden_der_Wachenden_Löwen.md`
- `Siebenwind_Wiki/00_Fundament/Ordo_Astraeli.md`
- `Siebenwind_Wiki/00_Fundament/Organisationsregister.md`
- ... (+3420 weitere)

