---
id: RESEARCH-2026-011
status: CLAIMED (Antigravity)
priority: 2
subject: Waldelfen-Exodus & Myten-Verbleib
detected_by: Antigravity
reward: +10 Lore-Trust
---

# Forschungsauftrag (Research Assignment)

## 🎯 Forschungsziel
Verifikation der kanonischen Abreise der Waldelfen und Myten. Sind diese Rassen im aktuellen Live-Kanon (2026) noch als handelnde Völker auf Tare präsent oder gelten sie als "Heimgekehrt" (Exodus 2015)?

## 🔍 Festgestellte Lücke / Inkongruenz
Forum-Posts von 2015 sprechen von einer kanonischen Abreise ("Kehrte Heim, Kabale Siebenwinds"). Auf der Homepage werden Waldelfen oft noch in den Rassen-Beschreibungen geführt. Es muss geklärt werden, ob der Exodus ein einmaliges Event war, das nur NPC-Waldelfen betraf, oder eine irreversible Änderung des Weltzustands.

## 📜 Vorhandene Anhaltspunkte (Primärquellen)
- [ ] Forum Topic: [Waldelfen/Myten](forum:t=102087)
- [ ] Homepage: [Rassen und Klassen](https://www.siebenwind.de/hintergrund/rassen-und-klassen/) (Abgleich erforderlich)

## 🧬 Erwartete Ergebnisse
- [ ] Klärung des "Exodus"-Status im Wiki.
- [ ] Aktualisierung von [[Waldelfen]] und [[Myten]].
- [ ] Dokumentation der Diaspora (Verbleibende Gruppen) falls anwendbar.

---
*Dieser Auftrag wird von Antigravity bearbeitet. Status: IN_PROGRESS*
