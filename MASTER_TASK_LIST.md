...
- **Status**: Die Forumfälle `MSG-2026-0216` bis `MSG-2026-0220` sind quellengewichtet integriert und abgeschlossen. Vier neue Erzählungsseiten sichern Sidras endophalische Perspektive, den namenlosen Ratsführer von Luth Chalid, den Bärenkampf und die offene Waldhütte; `Balian_Ruhnbert.md` erschließt den Handwerksmeister mit ausdrücklich begrenzter Verbindung zum Gesellschaftsentwurf. `audit --json` meldet `0` Issues und der Pages-Full-Smoke besteht. Der globale Pages-Snapshot liegt bei `625` unresolved / `623` unallowlisted und `drift_status = PASS`; zielbezogene Dry-Runs melden für alle neuen Seiten `0` Residuen, weshalb die globale Abweichung als `MSG-2026-0221` an den Techniker geroutet ist. Offen bleiben außerdem der fehlende Oracle-Index (`MSG-2026-0192`), der Translate-Datenpfad (`MSG-2026-0197`), die Laufzeit-/Timeout-Problematik der direkten Pages-Vollprüfung (`MSG-2026-0195`) und die Angamon-Kanonfrage (`MSG-2026-0204`).
...
- [ ] **Forum-Queue Batchweise verarbeiten**: Nach den Piloten weitere volltextarchivierte Forumquellen ueber `ingest forum-queue`, `forum-inspect`, `forum-draft` und `forum-finalize` bearbeiten. Am 2026-07-15 wurden zunaechst `110386` und `110339` integriert; der erste Fuenferbatch integrierte `110307`, `110186` und `110204` und routete `110194` / `109969` an den Historian. Der zweite Fuenferbatch integrierte `110166`, `110158`, `110155`, `110144` und `110037`. Der dritte Fuenferbatch integrierte `110019`, `109883` und `109927` und routete `109948` / `109946` an den Historian. Der vierte Fuenferbatch integrierte `105537`, `105810` und `105770`, archivierte `105844` ohne Wiki-Aenderung und routete `103300` an den Historian. Der fuenfte Fuenferbatch legte `Fjerulfs_Pfad` neu an, aktualisierte und deduplizierte `Vencurius` und archivierte `105760`, `105563` sowie `105756` ohne Wiki-Aenderung. Der sechste Fuenferbatch integrierte `105652` und `105592` und routete `105027`, `105420` sowie `105200` an den Historian. Sämtliche früheren Historiker-Nachläufe einschließlich `110194` sind abgeschlossen; danach wurden `105411`, `105233`, `105471` und `105386` integriert. Zwei weitere Fünferblöcke schlossen `105371`, `105249`, `105164`, `105219`, `105214` sowie `105159`, `105188`, `105132`, `105060` und `105023` ab. Als nächster vorgemerkter Board-Kandidat bleibt `104750`.
