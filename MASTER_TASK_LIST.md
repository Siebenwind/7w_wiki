...
- [ ] **Wissenswerk 2.0 Branch-First Konsolidierung**: Der generische Produktkern wird in diesem Branch professionalisiert, bevor ein sauberes Public-Repo extrahiert wird. Erledigt sind Root-Vertraege, `./wissenswerk.py`, RagPrep-Import, Provider-/Design-Smokes, englische Kernrollen (`coordinator`, `curator`, `verifier`, `maintainer`), `curate`, `doctor`, dry-run-first `reset`/`wipe`, `hygiene reports`, `export plan`, Memory-Hybrid-Policy, Gitignore-Hygiene und `wissenswerk_export_manifest.json`. Naechste Schritte: echte pgvector-Import-/Search-Implementierung, Artikelgenerierung aus Curation-Plaenen, generischer Paketlayout-Entwurf, Dispatch-Triage und schrittweises Verschieben der Siebenwind-Personae/Skills in Legacy- oder Tenant-Packs.
